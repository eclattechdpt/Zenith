...
export function SaleDetail({ saleId }: SaleDetailProps) {
  ...
  async function handleCancelReturn() {
    ...
    try {
      ...
      sileo.success({ title: "Devolucion cancelada", description: "Los movimientos de stock fueron revertidos" })
      ...
      setIsCancellingReturn(false)
      setCancelReturnTarget(null)
      sileo.error({ title: "Error al cancelar la devolucion" })
    }
  }
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex items-center gap-2 pl-10 sm:pl-0">
              ...
              >
                <Button
                  variant="outline"
                  className="group h-10 gap-2 rounded-xl px-5 text-sm font-bold sm:h-11 sm:px-6"
            ...
            {Number(sale.extra_amount) > 0 && (
              <div className="flex justify-between text-sm">
                <span className="flex items-center gap-1.5 text-sky-600">
                  {sale.extra_label?.trim() || "Servicio a domicilio"}
                </span>
                <span className="tabular-nums font-semibold text-sky-600">
                  +{formatCurrency(Number(sale.extra_amount))}
                </span>
              </div>
            )}
                ...
                <div
                  ...
                  className={`rounded-xl border p-4 ${
                    isCancelled
                      ...
                      : "border-rose-100 bg-gradient-to-b from-white to-rose-50/30"
                  }`}
                >
