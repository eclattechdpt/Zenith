...
export function ProductWizardDialog({ open, onClose }: ProductWizardDialogProps) {
  ...
  // "Filled" detection for auto-collapse
  ...
  // Auto-collapse info when user fills name and moves to pricing
  useEffect(() => {
    if (infoFilled && pricingOpen && !infoOpen) return // already collapsed
  }, [infoFilled, pricingOpen, infoOpen])
  ...
  async function onSubmit(data: CreateProductInput) {
    ...
    // (we'll upload after we have the product ID)
    const hasPendingFile = pendingFileRef.current !== null
    ...
    if (hasPendingFile && result.data?.id) {
      try {
        ...
        // Update the product with the real URL
  ...
  async function handleFormSubmit() {
    // Fill auto-generated fields directly into values (not via setValue)
  ...
  function handleImageChange(url: string | null) {
    // If it's a blob URL, store the file for deferred upload
    if (url?.startsWith("blob:")) {
      // Extract file from the hidden input (set by ProductImagePicker)
      // We need to get the file before it's lost
  ...
  const handleClose = useCallback(() => {
    ...
    const currentUrl = getValues("image_url")
        ...
        {!success && (
          <div className="flex flex-shrink-0 items-center justify-between border-b border-neutral-100 bg-white px-6 py-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)] sm:px-8">
            <div className="flex items-center gap-3">
              <ShoppingBag className="h-7 w-7 text-rose-400" strokeWidth={1.5} />
              <div>
            ...
            >
              ...
              >
                ...
                <p className="mt-1.5 text-sm text-neutral-500">
                  <span className="font-semibold text-neutral-700">{name}</span> se agrego al
                  catalogo
                </p>
              </motion.div>
              <motion.div
                    ...
                    <div className="flex flex-wrap items-center gap-4 sm:col-span-2">
                        ...
                        <Switch
                          ...
                          onCheckedChange={(c) => {
                            ...
                            if (!c) {
                              ...
                              // Auto-fill SKU on existing variant if empty
                              const f = cur[0] ?? { name: "", sku: "", price: 0, stock: 0, is_active: true }
                              if (!f.sku) {
                                const n = getValues("name")
                                const b = getValues("brand")
                                setValue("variants", [{ ...f, sku: generateSku(n, b) }], { shouldDirty: true })
                              }
                            }
                          }}
                        />
                        <Label htmlFor="wiz_variants" className="text-sm">
                          Tiene variantes
                        </Label>
                      </div>
                      <div className={`flex items-center gap-3 ${hasVariants ? "opacity-40" : ""}`}>
                        <Switch
                          ...
                          onCheckedChange={(c) => {
                            setValue("is_bundle", c, { shouldDirty: true })
                            if (c) setValue("has_variants", false, { shouldDirty: true })
                          }}
                        />
                        <Label htmlFor="wiz_bundle" className="text-sm">
                          Es un cofre
                        </Label>
                      </div>
                    </div>
                    ...
                    <div className="rounded-xl border border-white/80 bg-white p-4 shadow-sm">
                      <div className="mb-3 flex items-center gap-2">
                        <div className="flex h-6 w-6 items-center justify-center rounded-md bg-teal-50">
                          <PackageIcon className="h-3 w-3 text-teal-500" />
                        </div>
