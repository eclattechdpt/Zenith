"use client"

import { use } from "react"
import { useRouter } from "next/navigation"
import { motion } from "motion/react"

import { TableSkeleton } from "@/components/shared/loading-skeleton"
import { EmptyState } from "@/components/shared/empty-state"
import { CustomerForm } from "@/features/clientes/components/customer-form"
import { useCustomer } from "@/features/clientes/queries"
import type { CustomerInput } from "@/features/clientes/schemas"
import type { CustomerWithPriceList } from "@/features/clientes/types"

const containerVariants = {
  hidden: { opacity: 0 },
  visible: {
    opacity: 1,
    transition: { staggerChildren: 0.08, delayChildren: 0.1 },
  },
}

const itemVariants = {
  hidden: { opacity: 0, y: 20, filter: "blur(4px)" },
  visible: {
    opacity: 1,
    y: 0,
    filter: "blur(0px)",
    transition: { duration: 0.5, ease: [0.22, 1, 0.36, 1] as const },
  },
}

function customerToFormValues(
  customer: CustomerWithPriceList
): Partial<CustomerInput> {
  return {
    name: customer.name,
    phone: customer.phone,
    email: customer.email,
    address: customer.address,
    notes: customer.notes,
    price_list_id: customer.price_list_id,
  }
}

function EditCustomerContent({ id, onBack }: { id: string; onBack: () => void }) {
  const { data: customer, isLoading } = useCustomer(id)

  if (isLoading) return <TableSkeleton />

  if (!customer) {
    return (
      <EmptyState
        title="Cliente no encontrado"
        description="El cliente que buscas no existe o fue eliminado."
      />
    )
  }

  return (
    <CustomerForm
      customerId={id}
      defaultValues={customerToFormValues(customer)}
      onBack={onBack}
    />
  )
}

export default function EditClientePage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = use(params)
  const router = useRouter()

  return (
    <motion.div
      variants={containerVariants}
      initial="hidden"
      animate="visible"
      className="flex flex-col gap-6"
    >
      <motion.div
        variants={itemVariants}
        className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"
      >
        <div className="text-center sm:text-left">
          <h1 className="font-display text-2xl font-semibold tracking-tight text-neutral-950">
            Editar cliente
          </h1>
        </div>
      </motion.div>

      <motion.div variants={itemVariants}>
        <EditCustomerContent id={id} onBack={() => router.push("/clientes")} />
      </motion.div>
    </motion.div>
  )
}
