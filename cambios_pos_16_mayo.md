...
Última actualización: 2026-05-19 (v3 — cierre)
...
- **4.** Recibo de impresión rediseñado para impresora térmica (estilo Zara/Bodega Aurrera). El recibo HTML usado por `react-to-print` antes era colorido con fondos rosa, gradients y emojis — la térmica los renderiza como gris pálido que se desvanece. Nuevo formato: monospace (JetBrains Mono / Courier), puro negro sobre blanco, sin fondos, sin emojis. Bold solo en información clave (folio, total, headers de sección, %); regular para detalles. Separadores con dashed lines. Logos en B&W: nuevo `public/EclatLogo_Black.svg` (emblema + texto en `#000`, generado a partir del `DarkWithPink` original) para el header del ticket y como inline pequeño en "Powered by"; abbrixLogo.svg ya era 100% negro, se reusa como "Desarrollado por". El PDF descargable se ajustó por separado (ver 4b). Toda la información (folio, fecha, cliente, distribuidor, items con %/REGALO, subtotal, breakdown de descuentos, total, forma de pago, cambio, referencia) preservada. ✅
- **4b.** (Pulido del recibo) Logo del header del ticket bajado de 62mm a 38mm (a tamaño completo robaba la mitad del ancho). Bug arreglado en HTML+PDF: cuando una venta tenía descuento cart-level (`custom_amount`) sin descuentos per-item, el recibo mostraba un header "DESCUENTOS" sin renglones debajo y saltaba directo al TOTAL; ahora renderiza una línea única "Descuento (X%)" usando `discountPercent` del sale. ✅
- **4c.** (PDF) Tamaño cambiado de ticket térmico 80mm (que se descargaba como slip vertical raro) a hoja **carta** con el contenido en columna centrada de ~340pt; preservó el diseño colorido pero ahora se ve como nota elegante en hoja completa. Eliminada la lógica de altura dinámica que producía página 2 en blanco. `flexShrink` en el nombre del cliente arregla overflow para nombres largos (e.g. "MARIA DEL ROSARIO CORTEZ BORCEGUIN"). Quitado `fontStyle: italic` en la línea de referencia de pago — `PlusJakarta` no registra variante italic en `lib/pdf-fonts.ts` y crasheaba el PDF para ventas con transferencia/Otro con referencia. ✅
