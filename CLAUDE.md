...
**Cargo extra (servicio a domicilio)** — cargo con concepto editable (default "Servicio a domicilio") que **SUMA** al total (no descuenta). DB: `sales.extra_amount` + `sales.extra_label`; RPCs `create_sale_transaction` + `create_pending_sale` reciben `p_extra_amount`/`p_extra_label` (`complete_pending_sale` sin cambio — lee total de DB). Store `extra {amount,label}` incluido en `getTotal()`. Se captura en el paso de Pago (bloque sky, read-only en complete-pending); se imprime como línea `+$X` antes del TOTAL en térmico + PDF; aparece en totales de sale-detail/modal/confirmación. Disponible en ventas directas + pendientes (vale/split lo ignoran por diseño). Key files: `pos/components/{wizard-payment-step,pos-sale-wizard}.tsx`, `pos/store.ts`.

**Anotaciones de pago** — el campo de texto libre bajo cada pago (antes solo `transfer`/`other`) ahora también en `card`; reformulado "Anotación (opcional)"; prefijo en ticket `Ref:` → `Nota:`. **Gotcha (corregido)**: el snapshot del recibo en venta nueva omitía `reference` (solo complete-pending / reimpresión-DB lo llevaban) → ahora incluido para que la nota salga en el ticket impreso al instante.
...
- **Térmico** (`sale-receipt.tsx`): rediseñado **legibility-first** para térmicas 80mm (kit Seafon; área imprimible **72mm**, 203 DPI, **1-bit** = negro puro o nada). Imprime vía `react-to-print` → diálogo del navegador → **driver Windows** (NO ESC/POS crudo). Gotchas clave (para futuros cambios): **sans-serif pesado** (Arial/Helvetica — NO monospace; los trazos finos de Courier se desbaratan al binarizar), pesos **600/800** (el 400 sale débil), tamaños grandes (cuerpo 13px, TOTAL 20px, **nada <11px**), **ancho 72mm centrado** (80mm cortaba los bordes en el margen no imprimible), separadores = **regla negra sólida 2px** (no guiones), footer en **texto** (logos diminutos salían ilegibles), header logo 42mm, `#000` puro sin fondos/grises. Por ítem: `cant × precio = subtotal` (izq) + `- % desc = total` con el **importe final alineado a la derecha**; línea **"Piezas"** sobre el TOTAL. Tel `33 3167 7771`. Los 3 `pageStyle` (`pos-landing`, `sale-detail`, `sale-detail-modal`) resetean el margen del body. **✅ Validado en impresión real (Seafon, 23-may) — se imprime nítido y legible.**
  - **⏳ PENDIENTE (próxima sesión) — decisión de UX**: el descuento per-ítem ahora se ve **dos veces** cuando hay descuentos: en el renglón del producto (`- 40% = $444`) **y** en el bloque `DESCUENTOS` del footer del ticket. Falta decidir si **quitar el bloque `DESCUENTOS` del footer** (ya redundante) o conservarlo como desglose consolidado. Vive en `sale-receipt.tsx` (el IIFE de totales con los 3 branches A/B/C).
