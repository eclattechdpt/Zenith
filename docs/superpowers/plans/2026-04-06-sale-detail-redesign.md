# Sale Detail Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign the sale detail page to follow Design A patterns (PageHero-style header, SectionCards, KpiCards) with receipt-inspired content layout, plus add print/export via react-to-print.

**Architecture:** Single file rewrite of `sale-detail.tsx`. Reuses existing shared components (SectionCard, KpiCard) and the existing SaleReceipt print component. Actions moved to a shadcn DropdownMenu.

**Tech Stack:** React, Motion (framer-motion), shadcn/ui (Badge, Button, DropdownMenu), react-to-print, existing SaleReceipt component, SectionCard, KpiCard, date-fns, lucide-react.

---

### Task 1: Rewrite sale-detail.tsx

**Files:**
- Rewrite: `src/features/ventas/components/sale-detail.tsx`

- [ ] **Step 1: Replace the full content of `sale-detail.tsx`**
...
export function SaleDetail({ saleId }: SaleDetailProps) {
  ...
  const triggerPrint = useCallback(() => {
    ...
    // Small delay to ensure the hidden receipt is rendered
...
Run: `npm run dev` and navigate to a sale detail page (e.g. `/ventas/<any-sale-id>`).

Expected: Page renders with new Design A layout — PageHero-style header, 3 KPI cards, receipt-inspired items section, payment section, print button works.

- [ ] **Step 3: Commit**

```bash
git add src/features/ventas/components/sale-detail.tsx
git commit -m "feat: redesign sale detail — Design A + receipt-inspired layout + print"
```
