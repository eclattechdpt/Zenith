...
- **Test plan run 23-may** (2026-05-23): `TEST-PLAN-2026-05-19.md` now **168/185 ✅, 0 partials**, via code-audit + Playwright + Supabase SQL. Verified live: POS landing/wizard/discard, variant picker, ventas KPIs reactivity, inventario hub math, networks UI, receipt discount breakdown (V-0048 on-screen + PDF: `% (40%)` + `★ REGALO`, total $444), nav sweep (11 routes, 0 console errors), nuqs persistence. 21 remaining: §10 thermal visual (reworked, pending print-test), §8.20 mobile, §12.3-12.9 (e2e/cross-tab), §13.8-13.11/13.13 (transactional/print — not run to avoid DB mutation).
- **Thermal receipt legibility rework for Seafon 80mm** (2026-05-23): reworked `sale-receipt.tsx` for sharp thermal output (HTML print via react-to-print → Windows driver, not ESC/POS). Monospace→**bold sans** (thin Courier strokes break up at 1-bit/203 DPI), weights 600/800, bigger fonts (body 13px, TOTAL 20px, nothing <11px), width 80mm→**72mm centered** (no edge clipping in the no-print margin), solid 2px black separators, footer in text (tiny logos were illegible). Per-item layout: `qty × price = subtotal` (left) + `- % = total` with the **final amount right-aligned**; added **"Piezas"** line above TOTAL; phone → `33 3167 7771`. All 3 pageStyles reset body margin. **Pending: physical print-test on the Seafon** to fine-tune sizes/width. Note: per-item discount now also shows in the footer `DESCUENTOS` block (potential redundancy — left in pending a decision).

- **Extra charge (servicio a domicilio) + payment annotations** (2026-07-08): three POS receipt changes.
  - **Extra charge**: new editable-concept charge (default "Servicio a domicilio") that ADDS to the sale total. DB: `sales.extra_amount` + `sales.extra_label`; RPCs `create_sale_transaction` + `create_pending_sale` gained `p_extra_amount`/`p_extra_label` (`complete_pending_sale` unchanged — reads total from DB). Store `extra {amount,label}` folded into `getTotal()`. Input lives in the payment step (sky-themed, read-only in complete-pending); prints as `+$X` line before TOTAL on thermal + PDF; shown in sale-detail/modal/confirmation totals. Available in direct + pending sales (vale/split ignore it by design).
  - **Payment annotations**: the free-text field (was `transfer`/`other` only) now also shows for `card`; reworded to "Anotación (opcional)"; ticket prefix `Ref:` → `Nota:`. Fix: new-sale receipt snapshot was dropping `reference` (only complete-pending/DB-reprint carried it) — now included so the note prints on the immediately-printed ticket.
  - **Receipt location**: header line `Zapopan, Jalisco` → `IDEAL Guadalajara`.
